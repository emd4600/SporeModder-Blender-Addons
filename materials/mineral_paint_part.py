...
# Regions where the paint texture is verified to apply
...
# Regions where the color shows up dark in build mode
TEXTURE_TRIM_REGIONS = [
	4,
	16
]
	...
	def draw_panel(layout, rw4_material):
		...
		if data.paint_mode == 'TEXTURE':
			...
			if data.paint_mode == 'PAINT':
				if data.paint_region not in VALID_TEXTURE_REGIONS.keys():
					layout.label(text="Paint Texture may not work depending on the region.", icon='ERROR')
				else:
					layout.label(text="Paint Texture will work with this region.", icon='INFO')
				
				if data.paint_region in TEXTURE_TRIM_REGIONS:
